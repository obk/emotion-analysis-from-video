...
def run_analysis(video_path):
    try:
        ...
        for sec, emotion_data in emotions_per_second.items():
            dominant_emotion = max(emotion_data.keys(), key=lambda k: sum(emotion_data[k]) / len(emotion_data[k]))
            if previous_dominant_emotion is not None and dominant_emotion != previous_dominant_emotion:
                print(f"Second {sec}: Emotion changed from {previous_dominant_emotion} to {dominant_emotion}")
